  ...
  <header className="fixed top-0 left-0 right-0 bg-white border-b border-slate-100 z-50">
    <div className="max-w-[1600px] mx-auto px-6 h-[72px] flex items-center justify-between">
      ...
      <div className="flex items-center gap-2.5">
        <div className="w-10 h-10 bg-[#008c8c] rounded-[10px] flex items-center justify-center flex-shrink-0">
          <span className="text-white text-lg font-black">P</span>
        </div>
        <div className="flex flex-col">
           <span className="text-[18px] font-black text-[#1e293b] leading-none tracking-tight">PanKbase</span>
           <span className="text-[9px] text-[#94a3b8] font-bold tracking-[0.05em] mt-0.5 uppercase">KNOWLEDGE ENGINE</span>
        </div>
      </div>

      {/* Right: Navigation */}
      <div className="flex flex-col items-end">
        <div className="flex items-center gap-5 mb-1.5 text-[11px] font-semibold text-slate-800">
           <a href="#" className="flex items-center gap-1.5 hover:text-[#008c8c]">Funding</a>
           <a href="#" className="flex items-center gap-1 text-slate-300">Search</a>
           <a href="#" className="text-slate-300">Analysis</a>
           <a href="#" className="flex items-center gap-1.5 text-slate-300">Login</a>
        </div>

        <div className="flex items-center gap-4">
           <a href="#" className="h-8 px-4 bg-[#008c8c] text-white flex items-center justify-center rounded-[8px] font-bold text-[13px]">
              PanKGraph
           </a>
           <a href="#" className="text-[#008c8c] font-bold text-[13px] hover:underline">Integrated Cell Browser</a>
           <div className="h-4 w-[1px] bg-slate-200 mx-0.5"></div>
           <nav className="flex items-center gap-4 font-bold text-[13px] text-slate-800">
              <a href="#" className="hover:text-[#008c8c]">Data</a>
              <a href="#" className="hover:text-[#008c8c]">Resources</a>
              <a href="#" className="hover:text-[#008c8c]">About</a>
              <a href="#" className="hover:text-[#008c8c]">Help</a>
           </nav>
           <div className="ml-2 flex items-center gap-1.5">
              <div className="relative w-7 h-7">
                 <div className="absolute inset-0 rounded-full border border-dotted border-teal-400"></div>
                 <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-1 h-1 rounded-full bg-cyan-400 absolute top-0"></div>
                    <div className="w-1 h-1 rounded-full bg-green-500 absolute bottom-1 right-1"></div>
                    <div className="w-1 h-1 rounded-full bg-yellow-400 absolute bottom-1 left-1"></div>
                 </div>
              </div>
              <span className="text-[16px] font-black italic tracking-tighter text-[#1e293b]">HiRN</span>
           </div>
        </div>
      </div>
    </div>
...
const Sidebar: React.FC<{ turns: TurnData[], activeTurnId: string, activeSectionType: string, onNavClick: (tid: string, st?: string) => void }> = ({ turns, activeTurnId, activeSectionType, onNavClick }) => {
  const sections = [
        ...
        <div key={turn.id} className="flex flex-col gap-1">
          {turns.length > 1 && (
            <button 
              ...
              className={`text-left transition-all mb-1 ${activeTurnId === turn.id ? 'opacity-100' : 'opacity-40 hover:opacity-100'}`}
            >
              <div className="flex items-center gap-2 mb-1.5">
                <span className={`text-[10px] font-black px-2 py-0.5 rounded-[8px] ${activeTurnId === turn.id ? 'bg-[#008c8c] text-white' : 'bg-slate-200 text-slate-600'}`}>
                  Q{idx + 1}
                </span>
              </div>
              <p className={`text-[13px] font-bold leading-tight line-clamp-2 px-0.5 ${activeTurnId === turn.id ? 'text-[#1e293b]' : 'text-[#64748b]'}`}>
                {turn.query}
              </p>
            </button>
          )}
          ...
          {(turns.length === 1 || activeTurnId === turn.id) && (
            <div className={`relative ${turns.length > 1 ? 'ml-2 mt-0.5' : 'ml-0 mt-0'} flex flex-col gap-0 animate-in fade-in duration-300`}>
              <div className="absolute left-[3px] top-2 bottom-2 w-[1px] bg-slate-100"></div>
              {sections.map(s => (
                <button 
                  key={s.id} 
                  onClick={() => onNavClick(turn.id, s.id)} 
                  className={`relative text-left h-8 flex items-center gap-4 transition-all group ${activeSectionType === s.id ? 'text-[#008c8c]' : 'text-[#94a3b8] hover:text-[#64748b]'}`}
                >
                  <div className="flex items-center justify-center w-[7px] shrink-0 z-10">
                    <div className={`rounded-full transition-all duration-200 ${activeSectionType === s.id ? 'w-2 h-2 bg-[#008c8c]' : 'w-1 h-1 bg-slate-200'}`}></div>
                  </div>
  ...
  <div className="flex flex-col gap-6">
    <div>
      <div className="flex items-center gap-2 mb-2">
        <h4 className="text-[#008c8c] text-[10px] font-extrabold tracking-[0.05em] uppercase">GENE OVERVIEW:</h4>
      </div>
    ...
    <div>
      <div className="flex items-center gap-2 mb-2">
        <h4 className="text-[#008c8c] text-[10px] font-extrabold tracking-[0.05em] uppercase">QTL OVERVIEW:</h4>
      </div>
      <p className="text-[#475569] leading-relaxed text-[14px] font-normal text-justify">
    ...
    <div>
      <div className="flex items-center gap-2 mb-2">
        <h4 className="text-[#008c8c] text-[10px] font-extrabold tracking-[0.05em] uppercase">SPECIFIC RELATION TO TYPE 1 DIABETES:</h4>
      </div>
      <p className="text-[#475569] leading-relaxed text-[14px] font-normal text-justify">
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center gap-2 mb-1.5 overflow-x-auto scrollbar-hide">
        <TabButton isActive={activeTab === 'knowledge'} onClick={() => setActiveTab('knowledge')} label="Knowledge Graph" icon={<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><circle cx="5" cy="5" r="3"/><circle cx="19" cy="19" r="3"/><path d="M7.5 7.5l9 9"/></svg>} />
        <TabButton isActive={activeTab === 'empirical'} onClick={() => setActiveTab('empirical')} label="Empirical Evidence" icon={<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6"/></svg>} />
      </div>
      ...
      <div className="relative w-full aspect-[21/7] bg-white rounded-[20px] border border-slate-100 overflow-hidden">
        <div className="absolute top-4 left-6 right-6 flex justify-between items-center z-10">
          <button className="flex items-center gap-2 px-2.5 py-1 hover:bg-slate-50 rounded-md transition-colors">
            <svg className="w-3.5 h-3.5 text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M15 3h6v6M9 21H3v-6" /></svg>
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-60">
           <svg className="w-3/4 h-3/4" viewBox="0 0 800 300">
             ...
             <g transform="translate(320, 100)"><circle r="8" fill="white" stroke="#3b82f6" strokeWidth="2" /><text x="15" y="4" className="text-[12px] font-bold fill-slate-400">Pancreas</text></g>
             <g transform="translate(290, 210)"><circle r="8" fill="white" stroke="#f97316" strokeWidth="2" /><text x="15" y="4" className="text-[12px] font-bold fill-slate-400">rs2402203</text></g>
             <g transform="translate(410, 150)"><circle r="10" fill="white" stroke="#008c8c" strokeWidth="2" /><text x="20" y="5" className="text-[14px] font-black fill-slate-600">CFTR</text></g>
           </svg>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center gap-3 mb-1.5 flex-wrap">
        <TabButton 
          ...
          label="Reference" 
          icon={<svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M4 19.5A2.5 2.5 0 016.5 17H20M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg>} 
        />
        <TabButton 
        ...
        <TabButton 
          ...
          icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>} 
        />
        <TabButton 
          isActive={activeTab === 'external-links'} 
          onClick={() => setActiveTab('external-links')} 
          label="External Links" 
          icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>} 
        />
      </div>
      ...
      <div className="grid gap-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start gap-4 mb-0.5">
                ...
                <button className="flex items-center gap-1 px-2 py-1 rounded-md bg-slate-50 text-slate-500 hover:text-[#008c8c] hover:bg-teal-50 text-[10px] font-bold shrink-0">
                  <svg className="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" /></svg>
                  Cite
                </button>
              </div>
            ...
            <a href="#" className="p-3 bg-white border border-slate-100 rounded-xl hover:border-teal-200 transition-colors flex items-center justify-between group shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-teal-50 flex items-center justify-center text-[#008c8c]">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M7 21h10M12 3v18M3 12h18"/></svg>
                </div>
                <div className="flex flex-col">
              ...
              <svg className="w-3.5 h-3.5 text-slate-300 group-hover:text-[#008c8c]" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"/></svg>
            </a>
            <a href="#" className="p-3 bg-white border border-slate-100 rounded-xl hover:border-teal-200 transition-colors flex items-center justify-between group shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-teal-50 flex items-center justify-center text-[#008c8c]">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M2.45 14.97c.56 1.7 1.5 3.2 2.7 4.5l1.4-1.4m11.1-12.2l1.4-1.4c1.2 1.3 2.1 2.8 2.7 4.5m-15.4 0c-.6 1.7-.9 3.5-.9 5.4 0 1.9.3 3.7.9 5.4m15.4 0c.6-1.7.9-3.5.9-5.4 0-1.9-.3-3.7-.9-5.4"/></svg>
                </div>
                <div className="flex flex-col">
        ...
        {activeTab === 'external-links' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <a href="https://gtexportal.org" target="_blank" className="p-3 bg-white border border-slate-100 rounded-xl hover:border-orange-100 transition-colors flex items-center justify-between group shadow-sm">
              <div className="flex items-center gap-3">
            ...
            <a href="https://pubmed.ncbi.nlm.nih.gov" target="_blank" className="p-3 bg-white border border-slate-100 rounded-xl hover:border-blue-100 transition-colors flex items-center justify-between group shadow-sm">
              <div className="flex items-center gap-3">
...
const FollowUpContent: React.FC<{ questions: string[], onAsk: (q: string) => void }> = ({ questions, onAsk }) => (
  <div className="flex flex-col gap-1.5">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
      {questions.map((q, idx) => (
        <button key={idx} onClick={() => onAsk(q)} className="text-left p-3 rounded-[12px] border border-slate-100 bg-white hover:border-teal-200 hover:bg-teal-50/10 transition-all group flex justify-between items-center">
          <span className="text-slate-800 text-[12px] font-bold group-hover:text-[#008c8c]">{q}</span>
          <svg className="w-3 h-3 text-[#008c8c] opacity-0 group-hover:opacity-100 transition-all shrink-0" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
        </button>
      ))}
    </div>
  </div>
        ...
        <div className="flex gap-6 max-w-[1600px] w-full px-6">
          <aside className="hidden lg:block w-56 flex-shrink-0">
            <div className="sticky top-[92px]">
              <Sidebar turns={turns} activeTurnId={activeTurnId} activeSectionType={activeSectionType} onNavClick={scrollTo} />
            </div>
          </aside>
            ...
            {turns.map((turn, idx) => (
              <div key={turn.id} className="bg-white rounded-[20px] border border-slate-200/60 overflow-hidden flex flex-col shadow-sm">
                <div className="bg-slate-50/30 px-6 py-4 border-b border-slate-100">
                  <span className="text-slate-400 text-[8px] font-black uppercase tracking-widest block mb-1">Question {idx + 1}</span>
                  <h2 className="text-[18px] font-extrabold text-slate-900 leading-tight tracking-tight">{turn.query}</h2>
                </div>
                <div className="px-6 py-6 flex flex-col gap-6">
                  <section ref={el => { if (!sectionsRefs.current[turn.id]) sectionsRefs.current[turn.id] = {}; sectionsRefs.current[turn.id]['ai-overview'] = el; }}><AIOverviewContent data={turn.aiOverview} /></section>
                  <section ref={el => { if (sectionsRefs.current[turn.id]) sectionsRefs.current[turn.id]['visualizations'] = el; }}><VisualContent /></section>
                  <section ref={el => { if (sectionsRefs.current[turn.id]) sectionsRefs.current[turn.id]['evidence'] = el; }}><EvidenceContent citations={turn.citations} /></section>
                  <section ref={el => { if (sectionsRefs.current[turn.id]) sectionsRefs.current[turn.id]['follow-up'] = el; }}><FollowUpContent questions={turn.followUpQuestions} onAsk={handleFollowUp} /></section>
                </div>
              </div>
            ))}
          </main>
        ...
        <div className="flex gap-6 max-w-[1600px] w-full">
          ...
          <div className="flex-1 flex justify-center pointer-events-auto">
            <div className="bg-white/95 backdrop-blur rounded-[16px] p-1 border border-slate-200 flex items-center gap-3 h-12 w-full max-w-[600px] pl-4 pr-1 shadow-lg shadow-slate-200/50">
              <svg className="w-3.5 h-3.5 text-slate-300 ml-1" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
              <input type="text" placeholder="Ask a follow-up..." className="flex-1 bg-transparent border-none outline-none text-slate-800 text-[13px] font-semibold placeholder-slate-400" />
              <button className="h-9 px-4 bg-[#008c8c] text-white rounded-[12px] flex items-center gap-1.5 hover:bg-teal-700 transition-all font-bold text-[11px] shadow-sm">
                Ask AI <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
              </button>
            </div>
          </div>
        </div>
